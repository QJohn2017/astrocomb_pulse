...
%T = (0.5/uWave_freq)*1E12;     % Time window, unit ps, should be long enough for ~2 cycles
%T = 125;
T = 62;                    % Time window, unit ps, should be long enough for ~2 cycles
...
u_ini = circshift(u_ini, length(t)/2);  %Shift the waveform to the center of the time spectrum
...
z = 10e-3;
%betap = [0, 0, -1*4.38E-2, -2.41e-4, 3.16e-6];
betap = [0, 0, -1*4.38E-2, -2.41e-4, 3.16e-6];
%betap = [0, 0, -0.0217, 5.0308e-05];     % Dispersion Beta Params as [0, 0, beta2, beta3] unit [0 0 ps2/m ps3/m] 
alpha = 0.1/1000/4.34;     % Loss coefficient, unit m-1. Leading number is loss in dB/km
n2 = 3E-19;                % n2 of materialm m2/W
Aeff = 1E-6*1E-6;             % Effective Area of mode, m^2
gamma = 2*pi*n2/Aeff/lambda0*1e9; % nonlinearity coefficient 1/(W m)
%gamma = 0.78*1E-3;	%Nonlinaer Coefficient of material Taken from https://ieeexplore.ieee.org/document/7764544
fiber_loss = 0.7;    %Loss after compressor, etc
trans = 0.2;    %Transmission from lens to waveguide
Energy = fiber_loss*sqrt(trans)*ave_power(2)/uWave_freq*1E12; % Pulse Energy, pJ
peak_power = (Energy*1E-12)/(2*comp_fit.c1*1E-12);   %Peak power in J(?)
u_ini = comp_t(:, end); %Initial pulse from previous pulse
...
wg_fit = fit(t(fit_mask), wg_power_t(fit_mask, end), 'gauss1'); %Fit of Intenisty FWHM
...
subplot(6, 1, 6)
    ...
    title('After Waveguide'); xlim([400 5000])
%% Functions
